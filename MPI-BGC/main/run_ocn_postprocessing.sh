...
dcount=2 # domain count
vcount=7 # var count

# -- Parameters for output folder (define only name of output folder):
year_start=2003 # first year
year_stop=2023  # last year
nlines=5        # numbers of line

# -- Output path for the results folder (path was changed because of security reasons):
pOUT=../scratch/evchur/RESULTS
fout=${pOUT}/MAIN_${year_start}_${year_stop}_${nlines}lines
...
do
    ...
    do
        ...
        python3 ./fire_xarray.py ${domain[${i}]} ${var[${j}]}
    done
done
