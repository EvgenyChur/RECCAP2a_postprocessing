...
# 2.1: Function --> get_upscaling_ba_veg_class. Take 0.25 grid and upscale it
#                   to 0.5 grid - parameter burned_area_by_vegetation class
def get_upscaling_ba_veg_class(
    ...
    for tstep in range(len(time_steps)):
        ...
        if lplot == True:
            ...
            tot_ba025.append(pd.concat([pd.Series(dt), pd.Series(ba_old.data.sum())], axis = 1))
            tot_ba05.append(pd.concat( [pd.Series(dt), pd.Series(ba_new.sum())     ], axis = 1))
            tot_baf025.append(pd.concat([pd.Series(dt), pd.Series(ba_frac025.data.sum())], axis = 1))
            tot_baf05.append(pd.concat( [pd.Series(dt), pd.Series(ba_frac05.data.sum())] , axis = 1))
...
if __name__ == '__main__':
    ...
    linfo = False
    # Do you want to get plots for burned area (only)
    lplot = True

    # -- Define user datasets (NDEP, MODIS datasets)
    name4ndep = 'NDEP'
    name4modis = 'MODIS'
    ...
    ndep_pin, ndep_param = get_path_in(ndep_datasets, 'ndep')
    esa_pin, esa_param   = get_path_in([f'BA_{name4modis}'] , 'burned_area')
    data_OUT = makefolder(output_path().get('lib4upscaling_support'))
    ...
    # -- Select research parameter:
    lparam = 'burned_area'                       # 3D field
    #lparam = 'burned_area_in_vegetation_class'  # 4D field

    ret_coef = 1e9  # return original units

    # ==============================  Main program  =====================
    print('START program')
    # -- Read data (NDEP, ESA-CCI MODIS)
    print(ndep_pin)
    ndep_data = xrlib.get_data(ndep_pin, ndep_datasets, name4ndep, ndep_datasets,
                         linfo = linfo, lresmp = False  )[0]
    esa_data  = xrlib.get_data(esa_pin , esa_datasets , 'burned_area', [lparam],
                         linfo = linfo, lresmp = False  )[0]
    ...
    if lparam == 'burned_area_in_vegetation_class':
        esa_ba, old_ba, old_frac, new_ba, new_frac = get_upscaling_ba_veg_class(
            esa_data, lparam, lreport = True, lplot = True)
    else:
        esa_ba = get_upscaling_ba(esa_data, lparam, lreport = True)
    ...
    if lplot == True and lparam == 'burned_area':
