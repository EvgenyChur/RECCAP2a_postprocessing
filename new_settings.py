...
---------- ---------- ----                                                   
    1.1    %(date) Evgenii Churiulin, Max Planck Institute for Biogeochemistry
           Initial release
"""
import creator_dict4maps as cdm
...
class Bulder_config_class:
    def __init__(self, **kwargs):
        ...
        self.tp_jul = self.jul_refer(kwargs.get('jul'))
        self.tp_orc = self.orc_refer(kwargs.get('jul'))
        self.tp_gfed41s = self.gfed41s_refer(kwargs.get('gfed41s'))
        self.tp_modis = self.modis_refer(kwargs.get('modis'))
        self.tp_gfed_tot = self.gfed_tot_refer(kwargs.get('gfed_tot'))
        self.tp_gfed_fl = self.gfed_fl_refer(kwargs.get('gfed_fl'))
        self.tp_avhrr = self.avhrr_refer(kwargs.get('avhrr'))
        self.tp_mod17a2 = self.mod17a2_refer(kwargs.get('mod17a2'))
        self.tp_mod17a3 = self.mod17a3_refer(kwargs.get('mod17a3'))
        self.tp_globmap = self.globmap_refer(kwargs.get('globmap'))
        self.tp_ltdr = self.ltdr_refer(kwargs.get('ltdr'))
    ...
    @__verify_input_period
    def jul_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [1980, 2024]
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits    
    
    @__verify_input_period
    def orc_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [1980, 2024]
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits    
    
    @__verify_input_period
    def modis_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [2003, 2020]
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits
    
    @__verify_input_period    
    def gfed41s_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [2003, 2016] # 2020 for fFire
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits
   
    @__verify_input_period
    def gfed_tot_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [2003, 2020]
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits
    
    @__verify_input_period
    def gfed_fl_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [2003, 2020]
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits

    @__verify_input_period
    def avhrr_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [2003, 2016] # 2020
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits

    @__verify_input_period
    def mod17a2_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [2003, 2020]
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits

    @__verify_input_period
    def mod17a3_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [2003, 2020]
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits

    @__verify_input_period
    def globmap_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [2003, 2020]
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits

    @__verify_input_period
    def ltdr_refer(self, ulimits: Optional[list[int,int]] = None):
        default_limits = [2003, 2020]
        ds_tlimits = ulimits if ulimits != None else default_limits
        return ds_tlimits    
    ...
    def user_settings(self) -> dict:
        ...
        cfg.add(
            'time_limits',{
                'burned_area' : {
                    'GFED4.1s' : [
                        self.tp_gfed41s[self.tstart],
                        self.tp_gfed41s[self.tstop],
                        ],     
                    'GFED_TOT' : [
                        self.tp_gfed_tot[self.tstart],
                        self.tp_gfed_tot[self.tstop],
                        ],
                    'GFED_FL' : [
                        self.tp_gfed_fl[self.tstart],
                        self.tp_gfed_fl[self.tstop],
                        ], 
                    'BA_MODIS' : [
                        self.tp_modis[self.tstart],
                        self.tp_modis[self.tstop],
                        ],
                    'BA_AVHRR' : [
                        self.tp_avhrr[self.tstart],
                        self.tp_avhrr[self.tstop],
                        ],
                ...
                'npp' : {
                    'MOD17A3' : [
                        self.tp_mod17a3[self.tstart],
                        self.tp_mod17a3[self.tstop],
                        ],
                    'OCN' : [self.tp_ocn[self.tstart], self.tp_ocn[self.tstop]],
                    'JUL' : [self.tp_jul[self.tstart], self.tp_jul[self.tstop]],
                    },
                'gpp' : {
                    'MOD17A2' : [
                        self.tp_mod17a2[self.tstart],
                        self.tp_mod17a2[self.tstop],
                        ],
                    'MOD17A3' : [
                        self.tp_mod17a3[self.tstart],
                        self.tp_mod17a3[self.tstop],
        ...
        ba_mean_creator = cdm.Map_settings( 'burned_area', 'mean' ,  0.0 , 0.30, 'hot_r' )
        ba_std_creator = cdm.Map_settings(  'burned_area', 'std'  ,  0.0 , 0.10, 'hot_r' )
        ba_trend_creator = cdm.Map_settings('burned_area', 'trend', -1e-3, 1e-3, 'RdBu_r')
        # -- cVeg properties:
        cveg_mean_creator = cdm.Map_settings( 'cVeg', 'mean' ,   0.0, 20.0 , 'gist_earth_r')
        cveg_std_creator = cdm.Map_settings(  'cVeg', 'std'  ,   0.0,  2.0 , 'gist_earth_r')
        cveg_trend_creator = cdm.Map_settings('cVeg', 'trend', -0.04,  0.04, 'RdBu_r')
        # -- GPP properties (Gross Primary Production):
        gpp_mean_creator = cdm.Map_settings( 'gpp', 'mean' ,   0.0, 4000.0, 'gist_earth_r')
        gpp_std_creator = cdm.Map_settings(  'gpp', 'std'  ,   0.0,  400.0, 'gist_earth_r')
        gpp_trend_creator = cdm.Map_settings('gpp', 'trend', -10.0,   10.0, 'RdBu_r')
        # -- NPP properties (Net primary production):
        npp_mean_creator = cdm.Map_settings( 'npp', 'mean' ,   0.0, 1500.0, 'gist_earth_r')
        npp_std_creator = cdm.Map_settings(  'npp', 'std'  ,   0.0,  200.0, 'gist_earth_r')
        npp_trend_creator = cdm.Map_settings('npp', 'trend', -10.0,   10.0, 'RdBu_r')
        # -- LAI properties (Leaf Area Index):
        lai_mean_creator = cdm.Map_settings( 'lai', 'mean' ,   0.0 , 6.0  , 'Greens')
        lai_std_creator = cdm.Map_settings(  'lai', 'std'  ,   0.0 , 3.0  , 'Greens')
        lai_trend_creator = cdm.Map_settings('lai', 'trend',  -0.04, 0.04 , 'PRGn')
        # -- NEE properties (Net Ecosystem Exchange):
        nee_mean_creator = cdm.Map_settings( 'nee', 'mean' , -150.0,  50.0, 'PRGn')
        nee_std_creator = cdm.Map_settings(  'nee', 'std'  ,    0.0, 200.0, 'Greens')
        nee_trend_creator = cdm.Map_settings('nee', 'trend',   -2.0,   2.0, 'RdBu_r')            
        # -- NPB properties (Net Biome Production):
        nbp_mean_creator = cdm.Map_settings( 'nbp', 'mean' ,  -60.0,  60.0, 'PRGn')
        nbp_std_creator = cdm.Map_settings(  'nbp', 'std'  ,    0.0, 150.0, 'Greens')
        nbp_trend_creator = cdm.Map_settings('nbp', 'trend',   -2.0,   2.0, 'RdBu_r') 
        # -- fFire properties (CO2 Flux to Atmosphere from Fire):
        ffire_mean_creator = cdm.Map_settings( 'fFire', 'mean' ,  0.0, 80.0, 'hot_r')
        ffire_std_creator = cdm.Map_settings(  'fFire', 'std'  ,  0.0, 60.0, 'hot_r')
        ffire_trend_creator = cdm.Map_settings('fFire', 'trend', -0.4,  0.4, 'RdBu_r')        
        ...
        # -- Burned area properties:
        ba_mean_creator = cdm.Map_settings( 'burned_area', 'mean' ,  0.0 , 0.05, 'hot_r')
        ba_std_creator = cdm.Map_settings(  'burned_area', 'std'  ,  0.0 , 0.03, 'hot_r')
        ba_trend_creator = cdm.Map_settings('burned_area', 'trend', -1e-4, 1e-4, 'RdBu_r')
        # -- cVeg properties:
        cveg_mean_creator = cdm.Map_settings( 'cVeg', 'mean' ,  0.0 , 10.0 , 'gist_earth_r')
        cveg_std_creator = cdm.Map_settings(  'cVeg', 'std'  ,  0.0 ,  1.5 , 'gist_earth_r')
        cveg_trend_creator = cdm.Map_settings('cVeg', 'trend', -0.04,  0.04, 'RdBu_r')
        # -- GPP properties (Gross Primary Production):
        gpp_mean_creator = cdm.Map_settings( 'gpp', 'mean' ,   0.0, 3000.0, 'gist_earth_r')
        gpp_std_creator = cdm.Map_settings(  'gpp', 'std'  ,   0.0,  300.0, 'gist_earth_r')
        gpp_trend_creator = cdm.Map_settings('gpp', 'trend', -10.0,   10.0, 'RdBu_r')
        # -- NPP properties (Net primary production):
        npp_mean_creator = cdm.Map_settings( 'npp', 'mean' ,  0.0, 1500.0, 'gist_earth_r')
        npp_std_creator = cdm.Map_settings(  'npp', 'std'  ,  0.0,  200.0, 'gist_earth_r')
        npp_trend_creator = cdm.Map_settings('npp', 'trend', -6.0,    6.0, 'RdBu_r')
        # -- LAI properties (Leaf Area Index):
        lai_mean_creator = cdm.Map_settings( 'lai', 'mean' ,  0.0 , 6.0 , 'Greens')
        lai_std_creator = cdm.Map_settings(  'lai', 'std'  ,  0.0 , 3.0 , 'Greens')
        lai_trend_creator = cdm.Map_settings('lai', 'trend', -0.03, 0.03, 'PRGn')
        # -- NEE properties (Net Ecosystem Exchange):
        nee_mean_creator = cdm.Map_settings( 'nee', 'mean' , -100.0,  50.0, 'PRGn')
        nee_std_creator = cdm.Map_settings(  'nee', 'std'  ,    0.0, 100.0, 'Greens')
        nee_trend_creator = cdm.Map_settings('nee', 'trend',   -2.0,   2.0, 'RdBu_r')            
        # -- NPB properties (Net Biome Production):
        nbp_mean_creator = cdm.Map_settings( 'nbp', 'mean' , -60.0, 60.0, 'PRGn')
        nbp_std_creator = cdm.Map_settings(  'nbp', 'std'  ,   0.0,100.0, 'Greens')
        nbp_trend_creator = cdm.Map_settings('nbp', 'trend',  -2.0,  2.0, 'RdBu_r') 
        # -- fFire properties (CO2 Flux to Atmosphere from Fire):
        ffire_mean_creator = cdm.Map_settings( 'fFire', 'mean' ,  0.0, 10.0, 'hot_r')
        ffire_std_creator = cdm.Map_settings(  'fFire', 'std'  ,  0.0, 10.0, 'hot_r')
        ffire_trend_creator = cdm.Map_settings('fFire', 'trend', -0.1,  0.1, 'RdBu_r')
        # -- Create list of settings:
        cfg.add(
            'clb_limits_EN', [
                ba_mean_creator.make_dict(),
        ...
        # -- Burned area properties:
        ba_mean_creator = cdm.Map_settings( 'burned_area', 'mean' , -0.001, 0.001, 'RdBu_r')
        ba_std_creator = cdm.Map_settings(  'burned_area', 'std'  , -0.03 , 0.03 , 'RdBu_r')
        ba_trend_creator = cdm.Map_settings('burned_area', 'trend', -1e-4 , 1e-4 , 'bwr')
        # -- cVeg properties:
        cveg_mean_creator = cdm.Map_settings( 'cVeg', 'mean' , -1.0 ,  1.0, 'PiYG')
        cveg_std_creator = cdm.Map_settings(  'cVeg', 'std'  , -0.6 ,  0.6, 'PiYG')
        cveg_trend_creator = cdm.Map_settings('cVeg', 'trend', -1e-3, 1e-3, 'PiYG')
        # -- GPP properties (Gross Primary Production):
        gpp_mean_creator = cdm.Map_settings( 'gpp', 'mean' , -40.0, 40.0, 'PiYG')
        gpp_std_creator = cdm.Map_settings(  'gpp', 'std'  , -20.0, 20.0, 'PiYG')
        gpp_trend_creator = cdm.Map_settings('gpp', 'trend',  -0.4,  0.4, 'PiYG')
        # -- NPP properties (Net primary production):
        npp_mean_creator = cdm.Map_settings( 'npp', 'mean' , -20.0, 20.0, 'PiYG')
        npp_std_creator = cdm.Map_settings(  'npp', 'std'  , -15.0, 15.0, 'PiYG')
        npp_trend_creator = cdm.Map_settings('npp', 'trend',  -0.4,  0.4, 'PiYG')
        # -- LAI properties (Leaf Area Index):
        lai_mean_creator = cdm.Map_settings( 'lai', 'mean' , -1.0 ,  1.0, 'PiYG')
        lai_std_creator = cdm.Map_settings(  'lai', 'std'  , -0.4 ,  0.4, 'PiYG')
        lai_trend_creator = cdm.Map_settings('lai', 'trend', -1e-3, 1e-3, 'PiYG')
        # -- NEE properties (Net Ecosystem Exchange):
        nee_mean_creator = cdm.Map_settings( 'nee', 'mean' , -30.0, 30.0, 'PiYG')
        nee_std_creator = cdm.Map_settings(  'nee', 'std'  , -30.0, 30.0, 'PiYG')
        nee_trend_creator = cdm.Map_settings('nee', 'trend',  -0.5,  0.5, 'PiYG')
        # -- NPB properties (Net Biome Production):
        nbp_mean_creator = cdm.Map_settings( 'nbp', 'mean' , -30.0, 30.0, 'PiYG')
        nbp_std_creator = cdm.Map_settings(  'nbp', 'std'  , -30.0, 30.0, 'PiYG')
        nbp_trend_creator = cdm.Map_settings('nbp', 'trend',  -0.4,  0.4, 'PiYG') 
        # -- fFire properties (CO2 Flux to Atmosphere from Fire):
        ffire_mean_creator = cdm.Map_settings( 'fFire', 'mean' , -10.0, 10.0, 'PiYG')
        ffire_std_creator = cdm.Map_settings(  'fFire', 'std'  ,  -6.0,  6.0, 'PiYG')
        ffire_trend_creator = cdm.Map_settings('fFire', 'trend',  -0.2,  0.2, 'PiYG')
        ...
        # -- Burned area properties:
        ba_mean_creator = cdm.Map_settings( 'burned_area', 'mean' , -0.05, 0.05, 'RdBu_r')
        ba_std_creator = cdm.Map_settings(  'burned_area', 'std'  , -0.03, 0.03, 'RdBu_r')
        ba_trend_creator = cdm.Map_settings('burned_area', 'trend', -1e-3, 1e-3, 'bwr')
        # -- cVeg properties:
        cveg_mean_creator = cdm.Map_settings( 'cVeg', 'mean' , -0.4,  0.4, 'PiYG')
        cveg_std_creator = cdm.Map_settings(  'cVeg', 'std'  , -0.2,  0.2, 'PiYG')
        cveg_trend_creator = cdm.Map_settings('cVeg', 'trend', 1e-3, 1e-3, 'PiYG')
        # -- GPP properties (Gross Primary Production):
        gpp_mean_creator = cdm.Map_settings( 'gpp', 'mean' , -40.0, 40.0, 'PiYG')
        gpp_std_creator = cdm.Map_settings(  'gpp', 'std'  , -20.0, 20.0, 'PiYG')
        ...
        npp_trend_creator = cdm.Map_settings('npp', 'trend',  -0.4,  0.4, 'PiYG')
        # -- LAI properties (Leaf Area Index):
        lai_mean_creator = cdm.Map_settings( 'lai', 'mean' , -2.0 ,  2.0, 'PiYG')
        lai_std_creator = cdm.Map_settings(  'lai', 'std'  , -0.4 ,  0.4, 'PiYG')
        lai_trend_creator = cdm.Map_settings('lai', 'trend', -1e-2, 1e-2, 'PiYG')
        # -- NEE properties (Net Ecosystem Exchange):
        nee_mean_creator = cdm.Map_settings( 'nee', 'mean' , -7.5, 7.5, 'PiYG')
        nee_std_creator = cdm.Map_settings(  'nee', 'std'  , -7.5, 7.5, 'PiYG')
        nee_trend_creator = cdm.Map_settings('nee', 'trend', -0.4, 0.4, 'PiYG')
        # -- NPB properties (Net Biome Production):
        nbp_mean_creator = cdm.Map_settings( 'nbp', 'mean' , -7.5, 7.5, 'PiYG')
        nbp_std_creator = cdm.Map_settings(  'nbp', 'std'  , -7.5, 7.5, 'PiYG')
        nbp_trend_creator = cdm.Map_settings('nbp', 'trend', -0.4, 0.4, 'PiYG')
        # -- fFire properties (CO2 Flux to Atmosphere from Fire):
        ffire_mean_creator = cdm.Map_settings( 'fFire', 'mean' , -4.0, 4.0, 'PiYG')
        ffire_std_creator = cdm.Map_settings(  'fFire', 'std'  , -2.0, 2.0, 'PiYG')
        ffire_trend_creator = cdm.Map_settings('fFire', 'trend', -0.1, 0.1, 'PiYG')
        # -- Create list of settings:
        cfg.add(
            'clb_diff_limits_EN', [
                ba_mean_creator.make_dict(),
